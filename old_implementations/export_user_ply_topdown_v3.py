#!/usr/bin/env python3
"""
User .ply -> pseudo-ScanNet + top-down draggable room layout exporter (v3).
...
def estimate_room_polygon_v3(points: np.ndarray, args: argparse.Namespace) -> Tuple[Polygon, Dict[str, object]]:
    z = points[:, 2]
...
def label_proposal(width: float, depth: float, height: float, z_min: float, z_max: float, band: str, room_poly: Polygon, center: np.ndarray) -> Tuple[str, float]:
    """Geometry-only semantic guess.

    This is intentionally more detailed than v2, but still explicitly a guess.
    """
    ...
    near_wall = boundary_dist <= 0.45

    # Tall, very small-footprint object: most likely a lamp/plant/stand.
    if area <= 0.18 and height >= 0.9:
        return "floor_lamp_or_tall_thin_object", 0.45

    # Bed/sofa-like low large object.
    if area >= 1.65 and 0.18 <= height <= 1.10 and long_side >= 1.55:
        if long_side >= 1.80 and short_side >= 0.80 and z_max <= 0.95:
            return "bed", 0.48
        return "sofa", 0.42

    # Counters/desks are usually near a wall and rectangular, with useful height.
    if near_wall and 0.55 <= z_max <= 1.35 and area >= 0.45 and aspect >= 1.35:
        if long_side >= 1.40 and short_side <= 0.90:
            return "kitchen_counter", 0.42
        return "work_desk", 0.40

    # Table/desk surfaces from tabletop band.
    if band in {"tabletop", "countertop"} and 0.45 <= z_max <= 1.35 and 0.18 <= area <= 2.50:
        if near_wall and area >= 0.45:
            return "desk", 0.38
        return "table", 0.38

    # Chair/stool-like.
    if 0.10 <= area <= 0.65 and 0.30 <= height <= 1.35:
        return "chair_or_stool", 0.32

    # Tall storage.
    if height >= 1.25 and area >= 0.15:
        if aspect >= 1.40 or near_wall:
            return "cabinet_or_shelf", 0.36
        return "shelf_or_bookcase", 0.34

    # Generic surface/furniture fallback.
    if area >= 0.45 and height <= 1.35:
        if near_wall:
            return "desk", 0.28
        return "table", 0.28
    if area >= 0.15:
        return "furniture_proposal", 0.22
    return "small_object_proposal", 0.18
...
def proposal_score(label: str, area: float, height: float, point_count: int, band_name: str) -> float:
    label_bonus = {
        ...
        "desk": 1.5,
        "table": 1.5,
        "floor_lamp_or_tall_thin_object": 1.2,
        "chair_or_stool": 1.0,
        "cabinet_or_shelf": 1.4,
        "shelf_or_bookcase": 1.4,
        "furniture_proposal": 0.5,
    }.get(label, 0.25)
    band_bonus = {
        "low_large": 0.8,
        "tabletop": 0.8,
...
def build_layout(args: argparse.Namespace) -> Tuple[SceneLayout, Dict[str, object], np.ndarray, Optional[np.ndarray], Dict[str, object]]:
    ...
    room_poly, room_debug = estimate_room_polygon_v3(aligned, args)
    ...
    metadata = {
        "source": "user_ply_geometry_pseudo_scannet_v3",
        "input_ply": str(ply),
...
def write_pseudo_scannet_files(out_dir: str | Path, scene_id: str, input_ply: str | Path, aligned: np.ndarray, rgb: Optional[np.ndarray], pseudo: Dict[str, object], copy_original: bool = True) -> Path:
    ...
    with (scene_dir / f"{scene_id}.aggregation.json").open("w", encoding="utf-8") as f:
        json.dump({"sceneId": scene_id, "appId": "user-ply-pseudo-scannet-v3", "segGroups": pseudo["seg_groups"]}, f, indent=2)
    ...
    with (scene_dir / f"{scene_id}.txt").open("w", encoding="utf-8") as f:
        ...
        f.write("source = user_ply_pseudo_scannet_v3\n")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Raw user .ply -> pseudo-ScanNet + top-down draggable layout exporter v3")
